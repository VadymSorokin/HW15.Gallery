	...
	fetch('https://jsonplaceholder.typicode.com/albums')
		...
		.then((album) => {
			for (let i = 0; i < album.length; i++) {
				const albumItem = `<li data-id-number="${[i + 1]}">${album[i].title}</li>`;
	...
	fetch(`https://jsonplaceholder.typicode.com/photos?albumId=${Number(albumId)}`)
		...
		.then((photoList) => {
			for (let i = 0; i < photoList.length; i++) {
				const photo = `<li><img src="${photoList[i].url} alt="photo from album ${albumId}"></li>`;
				albumPhotos.insertAdjacentHTML('afterbegin', photo);
			}
		})
}
